...
export const AgentSessionStartedV1Schema = z.object({ 
...
"started_at": z.any().describe("RFC3339 UTC timestamp at which the session began. Producers should set this to match the envelope `time` field.") }).strict().describe("Session-start payload.") }).and(z.any()).describe("Emitted when an agent session begins. Producer is typically the agent runtime itself (e.g., Claude Code via bloodbank/services/agent-hooks/claude/publish.py on SessionStart). Consumers track session lifecycle, attribute downstream events to a session, and aggregate per-session metrics.")
export type AgentSessionStartedV1 = z.infer<typeof AgentSessionStartedV1Schema>
