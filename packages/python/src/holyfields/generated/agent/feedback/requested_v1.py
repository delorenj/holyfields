...
class AgentFeedbackRequestedV1Source(BaseModel):
...
class AgentFeedbackRequestedV1Payload(BaseModel):
    agent_id: str = Field(..., description='AgentForge registry ID')
    message: str = Field(..., description='Message to send to the agent')
    letta_agent_id: str | None = Field(None, description='Optional Letta agent ID override')
    context: dict[str, Any] | None = Field(None, description='Optional context for the agent')
    tags: list[str] | None = Field(None, description='Tags for this feedback request')
...
class AgentFeedbackRequestedV1(BaseModel):
    ...
    event_type: Literal['agent.feedback.requested'] = Field(..., description='Event type discriminator')
    ...
    source: AgentFeedbackRequestedV1Source = Field(..., description='Metadata about the source that emitted this event')
    payload: AgentFeedbackRequestedV1Payload

    EVENT_TYPE: ClassVar[str] = 'agent.feedback.requested'
