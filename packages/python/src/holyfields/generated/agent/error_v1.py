...
class AgentErrorV1Source(BaseModel):
...
class AgentErrorV1Payload(BaseModel):
    ...
    context: str | None = Field(None, description='What was happening when the error occurred')
...
class AgentErrorV1(BaseModel):
    ...
    event_type: Literal['agent.error'] = Field(..., description='Event type discriminator')
    ...
    source: AgentErrorV1Source = Field(..., description='Metadata about the source that emitted this event')
    payload: AgentErrorV1Payload
