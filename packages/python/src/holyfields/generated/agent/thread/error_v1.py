...
class AgentThreadErrorV1Source(BaseModel):
...
class AgentThreadErrorV1Payload(BaseModel):
    provider: str
    model: str | None = None
    error_message: str
...
class AgentThreadErrorV1(BaseModel):
    ...
    event_type: Literal['agent.thread.error'] = Field(..., description='Event type discriminator')
    ...
    source: AgentThreadErrorV1Source = Field(..., description='Metadata about the source that emitted this event')
    payload: AgentThreadErrorV1Payload

    EVENT_TYPE: ClassVar[str] = 'agent.thread.error'
