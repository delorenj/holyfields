...
class SessionThreadMessageV1Source(BaseModel):
...
class SessionThreadMessageV1Payload(BaseModel):
    session_id: str
    ...
    model: str | None = None
    thinking_included: bool | None = None
...
class SessionThreadMessageV1(BaseModel):
    ...
    event_type: Literal['session.thread.message'] = Field(..., description='Event type discriminator')
    ...
    source: SessionThreadMessageV1Source = Field(..., description='Metadata about the source that emitted this event')
    payload: SessionThreadMessageV1Payload

    EVENT_TYPE: ClassVar[str] = 'session.thread.message'
