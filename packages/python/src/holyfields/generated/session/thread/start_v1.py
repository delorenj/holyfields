...
class SessionThreadStartV1Source(BaseModel):
...
class SessionThreadStartV1Payload(BaseModel):
    session_id: str
...
class SessionThreadStartV1(BaseModel):
    ...
    event_type: Literal['session.thread.start'] = Field(..., description='Event type discriminator')
    ...
    source: SessionThreadStartV1Source = Field(..., description='Metadata about the source that emitted this event')
    payload: SessionThreadStartV1Payload

    EVENT_TYPE: ClassVar[str] = 'session.thread.start'
