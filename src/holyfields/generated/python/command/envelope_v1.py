...
class CommandEnvelopeV1(BaseModel):
    ...
    issued_by: str = Field(..., description="Identity of the command issuer (agent name, 'system', or human name).")
    action: str = Field(..., description="Command action identifier (e.g., 'run_drift_check', 'assign_ticket').")
    command_payload: dict[str, Any] = Field(..., description="Action-specific data. Schema varies by action.")
    priority: Optional[str] = Field(None, description="Execution priority. 'critical' bypasses queue ordering.")
    ttl_ms: Optional[int] = Field(None, description="Time-to-live in ms. Command rejected if not acked within TTL. 0 = no expiry.")
