...
class CommandErrorV1(BaseModel):
    ...
    retry_after_ms: Optional[int | None] = Field(None, description="Suggested retry delay in ms. Only meaningful when retryable=true.", ge=0)
    fsm_version: Optional[int | None] = Field(None, description="FSM version after error transition. Null if error before FSM engagement (e.g., TTL expiry).", ge=1)
